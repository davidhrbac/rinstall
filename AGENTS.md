...
- Bastion base NIC order is fixed: `nodes[bastion.service_node].nics[0]` is customer/local and `[1]` is management. `network_connection_names` only renames runtime NetworkManager profiles; mapping order never defines VM NIC ordering or identity.
