...
- Do not add a `make infra-destroy` shortcut; document explicit `terraform plan -destroy` and `terraform destroy` commands instead so destructive infra removal stays deliberate.
...
- Use `make infra-plan ENV=envs/example`, `make infra-apply ENV=envs/example`, then `make bastion-configure`, `make node-prep`, `make rke2-install`, `make rancher-install`, and `make rancher-bootstrap` in that order; Rancher install/bootstrap prepare the RKE2 kubeconfig automatically.
