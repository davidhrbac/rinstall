...
- Infra Terraform is intended to use the `http` backend for GitLab Terraform state, but backend config is prepared manually by the operator and is not generated from `env.yaml`.
