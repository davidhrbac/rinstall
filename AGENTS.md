...
- If `ssh.jump_host` is set, generated SSH config includes `~/.ssh/config`; `bastion1`/`prom1` use `ProxyJump <jump_host>`, and Rancher nodes use `ProxyJump <jump_host>,bastion1` when `bastion_proxy_roles` includes `rancher`.
