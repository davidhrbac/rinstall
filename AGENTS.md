...
- Schema v1 requires `terraform.backend` with `type: gitlab`; its non-secret URL, project ID, and state name are declared in instance `config.yaml`. Production Terraform state is GitLab-managed through Terraform's HTTP backend; local Terraform state is not supported for provisioning. Rinstall derives the GitLab backend URLs, and credentials remain runtime-only.
