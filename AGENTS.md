...
- If `ssh.jump_host` is set, generated SSH config includes `~/.ssh/config`; target host entries use `ProxyCommand` for pyinfra compatibility. `bastion1`/`prom1` proxy through `<jump_host>`, and Rancher nodes proxy through `<jump_host>` then bastion when `bastion_proxy_roles` includes `rancher`.
...
- pyinfra inventory is phase-aware: bastion/Rancher Helm/bootstrap phases connect only to bastion, RKE2 phases only to relevant Rancher nodes, and node-prep to all local nodes.
...
- Add the vSphere route on bastion from `bastion.vsphere_route` using `bastion.vsphere_route_connection`; keep real route values in private env config, not committed examples.
