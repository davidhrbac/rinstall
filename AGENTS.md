...
- `infra.vsphere.clone_timeout` controls the vSphere VM clone timeout in minutes and defaults to `60`; it is separate from the provider API timeout.
...
- Schema v1 requires `terraform.backend` with `type: gitlab`; its non-secret URL and project ID are declared in instance `config.yaml`. The Terraform state name is derived automatically as `<environment.id>-infra`. Production Terraform state is GitLab-managed through Terraform's HTTP backend; local Terraform state is not supported for provisioning. Rinstall derives the GitLab backend URLs, and credentials remain runtime-only.
...
- Bastion `/etc/hosts` is rendered from the environment configuration: named local node records use `IP FQDN shortname` so the FQDN is canonical and the short name remains an alias; the Rancher URL round-robin records remain single-name aliases for every node with `role: rancher`.
- `make -f rinstall/Makefile bastion-configure` installs `clustershell` on the bastion and renders `/etc/clustershell/groups.d/local.cfg` as a complete static file from `config.nodes`; `rancher`, `prometheus`, and `all` contain only configured Rancher/Prometheus nodes, with no downstream or dynamic discovery.
...
- In the sanitized example, use VM template 1 for `bastion1` and `prom1`; use VM template 2 for local Rancher/RKE2 VMs. Downstream cluster VMs are managed separately.
...
- On the v0.4 maintenance line, Squid listens only on port `3128`; `bastion.squid_http_port` may be omitted or set to `3128`, and custom listener ports must fail configuration validation. Do not manage `http_port` in the distro Squid configuration.
- Optionally use `bastion.network_connection_names` to rename base NetworkManager connection IDs on the bastion, for example `ens192: local` and `ens224: mgmt`. Downstream Linux devices retain their guest names; their NetworkManager connection IDs are logical `vlan<VLAN>`.
...
- dnsmasq derives its interface policy from the bastion's current Ethernet devices: the base/customer and management interfaces are excluded from DHCP while remaining available for DNS. Downstream DHCP uses one common `/etc/dnsmasq.d/20-rinstall-dhcp.conf` policy file plus one `/etc/dnsmasq.d/dnsmasq-vlan<VLAN>.conf` file per network.
...
- VMware attaches one physical vNIC per downstream VMware portgroup and owns VLAN tagging; Linux does not create 802.1Q subinterfaces or rename kernel devices. Terraform reports the provider MAC, pyinfra resolves it to the actual guest device (for example `ens256`), and the complete NetworkManager profile uses logical connection ID `vlan<VLAN>`, that device, the provider MAC, the bastion address, and no default route. Never persist MACs, guest interface names, VM UUIDs, or vSphere MoRefs in `config.yaml`.
- Render NetworkManager and dnsmasq configuration declaratively as complete managed files, not append/patch operations.
- The common DHCP policy uses `bind-dynamic` and `dhcp-authoritative`, excludes base/customer and management interfaces with `no-dhcp-interface`, and leaves DNS available there. Per-VLAN ranges and router options use logical `set:vlan<VLAN>` and `tag:vlan<VLAN>` tags; do not use guest device names as DHCP tags. Omit DHCP option 6 so dnsmasq advertises its own address in the matching VLAN.
...
- Append new downstream networks and supported DHCP range/lease changes through the instance config. Removing, reordering, renaming, or moving an existing downstream network is unsupported in v0.3.0 and must fail safely. Downstream VMs belong to a separate Terraform workflow, so rinstall cannot infer whether a network is unused.
