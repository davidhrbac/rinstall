...
def test_proxy_upstream_is_optional_and_validates_endpoint_shape():
    config = raw_example()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"host": "user@proxy.example.internal", "port": 9090},
        {"host": "proxy.example.internal/path", "port": 9090},
...
def test_effective_vsphere_server_uses_config_before_runtime_environment():
    config = raw_example()
...
def test_effective_vsphere_server_uses_runtime_environment_when_configured_value_is_absent():
    config = raw_example()
