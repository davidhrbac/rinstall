...
def squid_transition_command(upstream_enabled):
    ...
    return "".join(
        [
            ...
            'sysconfig_tmp=$(mktemp /etc/sysconfig/.rinstall-squid.XXXXXX); ',
            'cp -a "$sysconfig" "$sysconfig_tmp"; ',
            f"if grep -Eq '^[[:space:]]*SQUID_CONF[[:space:]]*=' \"$sysconfig_tmp\"; then sed -i -E 's|^[[:space:]]*SQUID_CONF[[:space:]]*=.*$|{desired_line}|' \"$sysconfig_tmp\"; ",
            f'else printf \'\\n%s\\n\' {shlex.quote(desired_line)} >> "$sysconfig_tmp"; fi; ',
            'if command -v restorecon >/dev/null 2>&1; then restorecon -F "$sysconfig_tmp"; fi; ',
            wrapper_install,
            'mv -f "$sysconfig_tmp" "$sysconfig"; ',
