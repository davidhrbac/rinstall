...
- Keep real customer/internal hostnames, IPs, and SSH topology names out of committed examples; use a separate instance repository for real values.
...
- Infra Terraform defaults to local state for the committed scaffold. GitLab HTTP backend is optional; its declaration and Terraform working root are generated under the per-environment runtime directory, never written into the pinned engine checkout. Backend config is not generated from `env.yaml`.
