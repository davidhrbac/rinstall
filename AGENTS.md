...
- pyinfra discovers the guest device by its Terraform-provided MAC and renders a complete NetworkManager profile with connection ID `vlan<VLAN>`, the resolved guest device, the bastion address, and no default route. Never persist MACs, guest interface names, VM UUIDs, or vSphere MoRefs in `config.yaml`.
