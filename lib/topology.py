...
def configured_ssh_jump_hops(config):
    """Return configured jump structure without reading the operator SSH config."""
    ssh = config.get("ssh", {})
    ...
    def expand_alias(alias, active_aliases=()):
        ...
        nested = expand_expression(definition.get("proxy_jump"), (*active_aliases, alias))
        return (*nested, _SshJumpHop(alias, definition.get("hostname") or definition.get("host"), definition.get("port")))

    def expand_expression(expression, active_aliases=()):
        if not expression:
            return ()
        return tuple(hop for alias in str(expression).split(",") for hop in expand_alias(alias, active_aliases))

    if isinstance(jump_host, str):
        return expand_expression(jump_host)
    alias = jump_host.get("alias", "rancher-env-jump")
    return (*expand_expression(jump_host.get("proxy_jump"), (alias,)), _SshJumpHop(
        alias,
        jump_host.get("hostname") or jump_host.get("host"),
        jump_host.get("port"),
    ))
...
def _uniform_address_kind(values):
    kinds = {_address_kind(value) for value in values if value is not None}
    return next(iter(kinds)) if len(kinds) == 1 else SYMBOLIC_ADDRESS
...
def build_desired_topology(config):
    ...
    services.append(
        ServiceTopology(
            ...
            resolution=(RESOLVED if bastion_service_interface else PARTIAL),
        )
    )
    services.append(
        ServiceTopology(
    ...
    rancher_local_address_kind = _uniform_address_kind(rancher_local_addresses)
        ...
        EndpointTopology(
            ...
            resolutions=(
                EndpointResolutionTopology(
                    ...
                    address_kind=rancher_local_address_kind,
                ),
                EndpointResolutionTopology(
                    scope="external",
                    addresses=(),
            ...
            EndpointTopology(
                ...
                resolutions=(
                    EndpointResolutionTopology(
                        ...
                        provenance=CONFIGURED,
                        resolution=RESOLVED,
                        address_kind=_address_kind(address),
                        verification=UNVERIFIED,
                    ),
                ),
                ownership=REFERENCED_EXTERNAL,
            ...
            EndpointTopology(
                ...
                resolutions=(
                    EndpointResolutionTopology(
                        ...
                        provenance=CONFIGURED,
                        resolution=RESOLVED,
                        address_kind=_address_kind(downstream.gateway),
                        verification=UNVERIFIED,
                    ),
                ),
                ownership=REFERENCED_EXTERNAL,
    ...
    rancher_sources = tuple(
        ResolvedEndpoint(
            ...
            _address_kind(host.local_ip or host.primary_ip),
        )
        for host in rancher_hosts
    )
    connectivity_rules = []
    ...
    for rule_id in sorted(administrative_edges):
        ...
        if destination_reference.kind == "host":
            ...
            destination_resolved = ResolvedEndpoint(
                ...
                _address_kind(destination_host.ssh_target),
            )
            destination_ports = (22,)
        else:
        ...
        ConnectivityEndpoint(
            ...
            (
                ResolvedEndpoint(
                    ...
                    _address_kind(bastion_service_ip),
                ),
            ),
        ),
        ("TCP",),
            ...
            ConnectivityEndpoint(
                ...
                (
                    ResolvedEndpoint(
                        ...
                        _address_kind(primary_host.local_ip or primary_host.primary_ip),
                    ),
                ),
            ),
            ("TCP",),
        ...
        ConnectivityEndpoint(
            ...
            (
                ResolvedEndpoint(
                    ...
                    _address_kind(bastion.local_ip or bastion.primary_ip),
                ),
            ),
        ),
        ConnectivityEndpoint(
...
def _support_entity_label(reference, topology):
    ...
    if reference.id.startswith("l2:dhcp-client@vlan:"):
        return f"DHCP clients (VLAN {reference.id.rsplit(':', 1)[-1]})"
    if reference.id.startswith("network:downstream:"):
        return f"VLAN {reference.id.rsplit(':', 1)[-1]}"
    if reference.id.startswith("service:authoritative-dhcp@bastion:vlan:"):
        return "Same-VLAN bastion DHCP"
    return {
