...
- The configured bastion's primary interface is on the customer VLAN and has a static IP; configure its secondary management interface with static `nics[].cidr` addressing.
...
- `bastion.service_ip` defaults to the configured bastion's primary/customer IP; set it explicitly only when a different service address is needed so `dnsmasq` and `squid` do not use the management address.
...
- Set `nodes[bastion.service_node].nics[].connection_name` to the current source NetworkManager connection ID for each configured bastion base NIC. The mapped target, or the source when unmapped, is the final connection ID used by `bastion.vsphere_route_connection`; mapping order is never semantic.
