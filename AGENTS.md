...
- Do not add Rancher API resources, Fleet configuration, downstream cluster lifecycle, or Rancher/Kubernetes upgrades here; manage them in a separate per-environment Rancher Terraform project.
...
- When environment schema changes are introduced, add an immutable `environment.id` for artifact/runtime isolation and a validated `schema_version`; do not infer stable identity only from `$(notdir $(ENV))`.
- Prefer `allow_unverified_ssl: false` for vSphere. Any insecure TLS override must be explicit and justified in a private environment config.
- Rancher and cert-manager versions must resolve from one environment-config source of truth; do not introduce duplicate fallback versions across pyinfra and shell scripts.
